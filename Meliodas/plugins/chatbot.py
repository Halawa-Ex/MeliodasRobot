...
from Meliodas import *
from Meliodas.mongo import chatb
from Meliodas.plugins.antlangs import get_arg
...
async def szcbot(_, message: Message):
    ...
    if chat:
       ...
       affiliateplus = requests.get(f"https://api.affiliateplus.xyz/api/chatbot?message={text}&botname=Meliodas&ownername=@Denz&user=1")
       textmsg = (affiliateplus.json()["message"])
       if "Affiliate+" in textmsg:
        textmsg = textmsg.replace("Affiliate+", "Meliodas bot created by @OfficiallMeliodas")
       if "Lebyy_Dev" in textmsg:
        textmsg = textmsg.replace("Lebyy_Dev", "Denz kitting Owner of @OfficiallMeliodas")
...
**Language Support**
Meliodas AI chatbot support almost all languages in world .
